...
//       };
//       setNodes((nds) => [...nds, newNode]);
//       setLastNodePosition((pos) => ({ x: pos.x + 50, y: pos.y + 50 }));
//     },
//     [lastNodePosition]
//   );

//   const autoLayout = useCallback(() => {
//     const LAYER_VERTICAL_GAP = 100;
...
//     const roots = nodes.filter((node) => parentMap[node.id].length === 0);

//     // BFS to assign layers
//     const layers: FlowNode[][] = [];
//     const visited = new Set<string>();
//     const queue: Array<{ node: FlowNode; layer: number }> = roots.map((n) => ({
//       node: n,
//       layer: 0,
//     }));
...
//         const index = layers[layer].findIndex((n) => n.id === node.id);
//         if (index !== -1) {
//           const x = index * (NODE_WIDTH + NODE_HORIZONTAL_GAP);
//           const y = layer * (NODE_HEIGHT + LAYER_VERTICAL_GAP);
//           return {
//             ...node,
//             position: { x, y },
...
//         }
//       }
//       return node; // in case it's disconnected
//     });
...
//     const handleKeyDown = (e: KeyboardEvent) => {
//       if (e.key === "Delete") {
//         if (confirmDeleteNodeId) confirmDeleteNode();
//         else if (selectedNode) requestDeleteNode(selectedNode.id);
//         else if (edgeToDelete) handleDeleteEdge();
//       }
//     };
//     window.addEventListener("keydown", handleKeyDown);
//     return () => window.removeEventListener("keydown", handleKeyDown);
...
//       <div className="absolute top-2 left-1/2 transform -translate-x-1/2 z-30">
//         <div
//           className={`px-4 py-1 rounded-md text-sm font-medium shadow-md ${
//             saveStatus === "saving"
//               ? "bg-yellow-100 text-yellow-800"
//               : "bg-green-100 text-green-800"
//           }`}
...
//           <button
//             onClick={handleExport}
//             className="bg-gradient-to-r from-blue-600 to-blue-500 text-white px-5 py-2 rounded-md font-semibold shadow-md hover:from-blue-700 hover:to-blue-600 transition duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-1"
//           >
//             Export JSON
//           </button>
//           <label
//             htmlFor="import-json"
//             className="bg-gradient-to-r from-green-600 to-green-500 text-white px-5 py-2 rounded-md font-semibold shadow-md cursor-pointer hover:from-green-700 hover:to-green-600 transition duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-green-400 focus:ring-offset-1 select-none"
//           >
//             Import JSON
//           </label>
//           <input
      ...
      <div className="flex-1 h-full relative">
        ...
        >
          ...
          <MiniMap
            ...
            nodeBorderRadius={6}
            maskColor={
              darkMode ? "rgba(20, 20, 20, 0.5)" : "rgba(255, 255, 255, 0.4)"
            }
          />
        </ReactFlow>
        {edgeToDelete && (
